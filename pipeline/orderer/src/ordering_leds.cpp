...
void saveLedMap(std::string file_led_order, map<int, int> leds_world_to_trace_order) {
	std::ofstream outfile(file_led_order, std::ios::out | std::ios::trunc);
	int totalLeds = leds_world_to_trace_order.size();
...
int get_index(vector<size_t> edges, int edge) {
	int index = 0;
	for(auto e: edges) {
		if( e == edge) {
			return index;
...
int main(int argc, char **argv) {
	...
	if (argc == 1) {
		...
		file_sht = "../data/star-024_05_half.sheet";
		file_fis = "../data/star-024_05_half_1515_2i.trc";
	...
	std::vector<ExtendedVertex> vertex_order_contour = sh.get_vertex_order(0); // NOTE Generalize this for multi-patch function later
	...
	std::vector<FaceInfo> fis_ordered; 
	for(auto fi: t_fis_fids) {
		fis_ordered.push_back(fi.first);
	}
	...
	chainsPerFace.resize(fis_ordered.size()); // there are as many chains in a face as there are number of hinge edges.  
	// we have to circle around the hinge edges in order to uncover all of them

	float abs = 1e-8;
	int prev_module_count = 0; 
	for(auto fi: fis_ordered) {
		// if(fi.fid != 14) {continue; }
		printf("face id: %d\n", int(fi.fid));
		...
		bool has_connector = fi.has_connector;
